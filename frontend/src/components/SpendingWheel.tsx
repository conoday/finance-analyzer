...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div className="glass-strong rounded-xl p-3 text-xs min-w-[160px]">
      <p className="font-medium text-slate-200 mb-1">{d.kategori}</p>
...
export function SpendingWheel({ data }: SpendingWheelProps) {
  ...
  return (
    ...
    >
      <h2 className="text-sm font-semibold text-slate-200 mb-4">Distribusi Pengeluaran</h2>
        ...
        {top.slice(0, 3).map((row, i) => (
          <div key={row.kategori} className="flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-slate-400">{row.kategori}</span>
            </div>
            <span className="text-slate-300 font-mono">{formatRupiah(row.total, true)}</span>
          </div>
        ))}
      </div>
    </motion.div>
